...
const FeatureCard = ({ icon: Icon, title, shortDesc, fullDesc, image }: any) => {
  ...
  return (
    <div
      ...
      onClick={() => {
        // mobile-friendly toggle only
        if (window.innerWidth < 768) setIsOpen((v) => !v);
      }}
    >
      {/* Background Image */}
      <div className="absolute inset-0">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/80 to-black/30 opacity-90 transition-opacity duration-500 group-hover:opacity-85" />
        <div className="absolute inset-0 bg-[#b96384]/20 opacity-0 transition-opacity duration-500 group-hover:opacity-100 mix-blend-overlay" />
      </div>
      ...
      <div className="relative h-full p-8 md:p-10 flex flex-col justify-end z-10">
        ...
        <div className="transform transition-transform duration-500 ease-out translate-y-4 group-hover:translate-y-0">
          <h3 className="text-3xl font-bold font-[Space_Grotesk] text-white mb-2 drop-shadow-md">
            {title}
          </h3>
          ...
          <div className="relative">
            <div
              className={`transition-all duration-300 ease-out ${
                isOpen ? "h-0 opacity-0 overflow-hidden" : "h-auto opacity-100"
              }`}
            >
            ...
            <div
              className={`space-y-4 transition-all duration-500 ease-out ${
                isOpen ? "max-h-[200px] opacity-100 mt-2" : "max-h-0 opacity-0 overflow-hidden"
              }`}
            >
          ...
          {/* CTA */}
          <div
            className={`mt-6 flex items-center text-white font-semibold tracking-wide uppercase text-xs transition-all duration-500 delay-75 ${
              isOpen ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4"
            }`}
          >
            <span className="border-b border-transparent group-hover:border-[#b96384] pb-0.5 transition-colors">
              Explore service
            </span>
